...
public class Game extends JPanel implements Runnable, KeyListener, MouseListener, MouseMotionListener{
	...
	private ArrayList <EnemyProj> bullets;
	...
	public Game() {
		...
		bullets = new ArrayList<EnemyProj>();
	...
	public void paint(Graphics g){
		...
		if(gameState=="test room"){
			...
			setEnemyBullet(testroomEnemies, bullets, 10, 0);
			drawEnemyBullets(bullets, g2d);
	...
	private void setEnemyBullet(ArrayList<Enemy> enemy, ArrayList<EnemyProj> bullet, int damage, int roomid){
		Random rand = new Random(roomid);
		
		for(Enemy e: enemy){
			bullet.add(new EnemyProj(e.getCX(ppp),e.getCY(ppp), damage));
			bullet.get(bullet.size()-1).setUX(rand.nextInt(roomid));
			bullet.get(bullet.size()-1).setUY(rand.nextInt(roomid));
		}
